---
title: "Model Selection with the Fast and Robust Bootstrap"
...
robust model selection procedure 
for linear regression models based on MM-estimators and the 
...
```
The "classical" stepwise methods all yield the same
optimal submodel with 11 features:
```{r classical}
library(MASS)
...
```
Using a backwards stepwise 
approach with the Robust Future Prediction Error criterion yields the 
following model, also with 11 variables:
```{r RFPE}
p <- 14; model <- 1:14 ;
...
dimnames(a.aic$model)[[2]]
```
Now we use backward stepwise using 
Shao's and Welsh's criterion combined with the
fast and robust boostrap. We use `b = 1000` bootstrap samples
of size `nboot = 150`. 
```{r FRBmodelselection1}
b <- 1000; nboot <- 150; p <- 14
```
We generate the bootstrap samples, and set up
lists to store the optimal models 
to be found for each size 1, 2, ..., p. 
```{r FRBmodelselection2}
set.seed(123)
boot.samp <- matrix(sample(n, b*nboot, repl=TRUE), b, nboot)
model.s <- model.w <- 1:14
models.s <- models.w <- vector('list', length(model.s))
```
We will use a loss function in Tukey's bi-square family, 
and we do not want to compute the fully-bootstrapped
estimators:
```{r FRBmodelselection3}
k <- 1 ; rho.type <- 1 ; tr <- 2
my.ctrl <- rlm.control(M=b, calc.full=0)
criterias.s <- criterias.w <- rep(NA,14)
```
Now we fit the full model, and then 
proceed in a stewpwise manner:
```{r FRBmodelselection4}
...
model.rfpe
```
The models selected with these methods are slighlty different, but
noticeably smaller than those found using AIC/BIC or Mallow's Cp:
```{r sameagain}
